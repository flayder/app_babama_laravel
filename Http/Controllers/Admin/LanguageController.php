...
class LanguageController extends Controller
{
    ...
    public function delete(language $language)
    {
        ...
        if ('en.json' != $language->short_name.'.json') {
    ...
    public function keywordUpdate(Request $request, $id)
    {
        ...
        file_put_contents(resource_path('lang/').strtolower($lang->short_name).'.json', $content);
    ...
    public function storeKey(Request $request, $id)
    {
        ...
        $items = file_get_contents(resource_path('lang/').strtolower($lang->short_name).'.json');
        ...
        if (\array_key_exists($requestkey, json_decode($items, true))) {
            ...
            file_put_contents(resource_path('lang/').strtolower($lang->short_name).'.json', json_encode($result));
    ...
    public function deleteKey(Request $request, $id)
    {
        ...
        $lang = Language::findOrFail($id);
        $data = file_get_contents(resource_path('lang/').strtolower($lang->short_name).'.json');
    ...
    public function updateKey(Request $request, $id)
    {
        ...
        file_put_contents(resource_path('lang/').strtolower($lang->short_name).'.json', json_encode($jsonArray));
