...
class HomeController extends Controller
{
    ...
    public function calculation(Request $request)
    {
        ...
        if ('receive' == $request->sendReceive) {
    ...
    public function sendMoneyFormData(SendMoney $sendMoney, Request $request)
    {
        ...
        if (optional($sendMoney->provider)->services_form) {
            foreach ($sendMoney->provider->services_form as $key => $cus) {
                ...
                if ('file' == $cus->type) {
                    $rules[$key][] = trim($cus->validation);
                    $rules[$key][] = 'max:'.trim($cus->field_length);
                }
                if ('text' == $cus->type) {
                    $rules[$key][] = trim($cus->validation);
                    if ('max' == $cus->length_type) {
                        $rules[$key][] = 'max:'.trim($cus->field_length);
                    } elseif ('digits' == $cus->length_type) {
                        $rules[$key][] = 'digits:'.trim($cus->field_length);
                    }
                }
                if ('textarea' == $cus->type) {
                    $rules[$key][] = trim($cus->validation);
                    $rules[$key][] = 'max:'.trim($cus->field_length);
                }
                $inputField[] = $key;
    ...
    public function sendMoneyAction(SendMoney $sendMoney, $actionType)
    {
        ...
        if (0 == $sendMoney->status && 0 == $sendMoney->payment_status && 'fillup' == $actionType) {
            return redirect()->route('user.sendMoney', $sendMoney);
        } elseif (2 == $sendMoney->status && 0 == $sendMoney->payment_status && 'payment' == $actionType) {
            session()->put('invoice', $sendMoney->invoice);

            return redirect()->route('user.addFund');
        } elseif (0 != $sendMoney->status && 'details' == $actionType) {
            ...
            $status = '';
            if (0 == $sendMoney->status && 0 == $sendMoney->payment_status) {
                $status = 'Information Need';
            } elseif (2 == $sendMoney->status && 0 == $sendMoney->payment_status) {
                $status = 'Sender Not Pay Yet';
            } elseif (3 == $sendMoney->status || 2 == $sendMoney->payment_status) {
                $status = 'Cancelled';
            } elseif (1 == $sendMoney->status && 1 == $sendMoney->payment_status) {
                $status = 'Completed';
            } elseif (2 == $sendMoney->status && 1 == $sendMoney->payment_status) {
                $status = 'Processing';
            } elseif (2 == $sendMoney->status && 3 == $sendMoney->payment_status) {
                $status = 'Payment Hold';
            }
    ...
    public function fundHistorySearch(Request $request)
    {
        ...
        $funds = Fund::orderBy('id', 'DESC')->where('user_id', $this->user->id)->where('status', '!=', 0)
            ->when(isset($search['name']), fn ($query) => $query->where('transaction', 'LIKE', $search['name']))
            ->when(1 == $date, fn ($query) => $query->whereDate('created_at', $dateSearch))
    ...
    {
        $languages = Language::all()->map(fn ($item) => $item->id);
