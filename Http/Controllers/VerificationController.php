...
class VerificationController extends Controller
{
    ...
    public function checkValidCode($user, $code, $add_min = 10000)
    {
        ...
        if ($user->sent_at->addMinutes($add_min) < Carbon::now()) {
            return false;
        }
    ...
    public function check(): View|Factory|RedirectResponse|Application
    {
        $user = auth()->user();
        if (!$user->status) {
            Auth::logout();
    ...
    public function mailVerify(Request $request): RedirectResponse
    {
        $rules = [
            'code' => 'required',
        ];
    ...
    public function smsVerify(Request $request): RedirectResponse
    {
        $rules = [
            'code' => 'required',
        ];
