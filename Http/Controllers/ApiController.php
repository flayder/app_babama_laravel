...
class ApiController extends Controller
{
    ...
    public function apiV1(Request $request)
    {
        ...
        if ('balance' == strtolower($req['action'])) {
            $result['status'] = 'success';
            $result['balance'] = $user->balance;
            $result['currency'] = $basic->currency;

            return response()->json($result, 200);
        } elseif ('services' == strtolower($req['action'])) {
            ...
            return response()->json($result, 200);
        } elseif ('add' == strtolower($req['action'])) {
            $validator = Validator::make($req, [
                'service' => 'required',
                'link' => 'required',
            ...
            if ($service->min_amount <= $quantity && $service->max_amount >= $quantity) {
                ...
                $order->price = $price;
                $order->runs = $req['runs'] ?? null;
                $order->interval = $req['interval'] ?? null;
        ...
        } elseif ('status' == strtolower($req['action'])) {
            $validator = Validator::make($req, [
                'order' => 'required',
            ]);
            if ($validator->fails()) {
            ...
            return response()->json($result, 200);
        } elseif ('orders' == strtolower($req['action'])) {
            $validator = Validator::make($req, [
                'orders' => 'required',
            ]);
            if ($validator->fails()) {
