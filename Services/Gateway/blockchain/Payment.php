...
class Payment
{
    ...
    public static function ipn($request, $order = null)
    {
        $btc = $request->value / 100000000;
        if ($order->btc_amount == $btc && $request->address == $order->btc_wallet && $request->secret == $order->transaction && $request->confirmations > 2 && 0 == $order->status) {
