...
class BlockIo
{
    ...
    { // the constructor
        ...
        $this->withdrawal_methods = ['withdraw', 'withdraw_from_user', 'withdraw_from_users', 'withdraw_from_label', 'withdraw_from_labels', 'withdraw_from_address', 'withdraw_from_addresses'];

        $this->sweep_methods = ['sweep_from_address'];
    }
    ...
    private function _request($path, $args = [], $method = 'POST')
    {
        // Generate cURL URL
        ...
        if ('POST' == $method) { // this was a POST method
    ...
    private function _withdraw($name, $args = [])
    { // withdraw method to be called by __call
        ...
        if ('success' == $response->status && \array_key_exists('reference_id', $response->data)) { // we have signatures to append
            // get our encryption key ready
            if (0 == \strlen($this->encryption_key)) {
            ...
            $response = $this->_request('sign_and_finalize_withdrawal', ['signature_data' => $json_string]);
        }

        return $response;
    }

    private function _sweep($name, $args = [])
    { // sweep method to be called by __call
        ...
        if ('success' == $response->status && \array_key_exists('reference_id', $response->data)) { // we have signatures to append
            ...
            // let's send the signed data back to Block.io
            $response = $this->_request('sign_and_finalize_sweep', ['signature_data' => $json_string]);
        }
...
class BlockKey
{
    ...
    {
        $table = ['1', '2', '3', '4', '5', '6', '7', '8', '9', 'A', 'B', 'C', 'D',
    ...
    public function base58_decode($encodedData, $littleEndian = true)
    {
        ...
        $i = $length - 1;
        while ('1' == substr($encodedData, $i, 1)) {
            $res = '00'.$res;
            --$i;
        }
    ...
    {
        $p = $this->p;
        if (0 == gmp_cmp($pt1['x'], $pt2['x']) && 0 == gmp_cmp($pt1['y'], $pt2['y'])) { // if identical
    ...
    public function mulPoint($k, array $pG, $base = null)
    {
        ...
        if (16 == $base || null == $base || \is_resource($base)) {
            $k = gmp_init($k, 16);
        }
        if (10 == $base) {
            $k = gmp_init($k, 10);
    ...
    public function sqrt($a)
    {
        ...
        if (1 != gmp_legendre($a, $p)) {
            // no result
            return null;
        }

        if (3 == gmp_strval(gmp_mod($p, gmp_init(4, 10)), 10)) {
            $sqrt1 = gmp_powm(
                $a,
                gmp_div_q(
    ...
    public function calculateYWithX($x, $derEvenOrOddCode = null)
    {
        ...
        if (!$derEvenOrOddCode) {
            return $y;
        } elseif ('02' == $derEvenOrOddCode) { // even
            $resY = null;
            ...
            return $resY;
        } elseif ('03' == $derEvenOrOddCode) { // odd
            $resY = null;
    ...
    {
        if ('04' == substr($derPubKey, 0, 2) && 130 == \strlen($derPubKey)) {
            // uncompressed der encoded public key
            $x = substr($derPubKey, 2, 64);
            $y = substr($derPubKey, 66, 64);

            return ['x' => $x, 'y' => $y];
        } elseif (('02' == substr($derPubKey, 0, 2) || '03' == substr($derPubKey, 0, 2)) && 66 == \strlen($derPubKey)) {
    ...
    public function getDerPubKeyWithPubKeyPoints($pubKey, $compressed = true)
    {
        if (true == $compressed) {
            ...
            if (0 == gmp_strval(gmp_mod(gmp_init($pubKey['y'], 16), gmp_init(2, 10)))) {
                $pubKey = '02'.$pubKey['x'];
            }	// if $pubKey['y'] is even
            else {
                $pubKey = '03'.$pubKey['x'];
    ...
    public function setPrivateKey($k): void
    {
        ...
        if (1 == gmp_cmp(gmp_init($k, 16), gmp_sub($this->n, gmp_init(1, 10)))) {
    ...
    { // extract the private key from the key in Wallet Import Format
        if (false === $this->validateWifKey($pp)) {
        ...
        if ('01' == substr($withoutVersion, 64, 2)) { // is compressed
            $this->c = true;
     ...
     * @throws \Exception
     */
    public function generateRandomPrivateKey($extra = 'FSQF5356dsdsqdfEFEQ3fq4q6dq4s5d'): void
    {
        // private key has to be passed as an hexadecimal number
        ...
        } while (1 == gmp_cmp(gmp_init($this->k, 16), gmp_sub($this->n, gmp_init(1, 10))));
    }
