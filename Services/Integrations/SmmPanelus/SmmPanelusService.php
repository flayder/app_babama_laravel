...
namespace App\Services\Integrations\SmmPanelus;
...
/**
 * Сервис для работы с SmmPanelus
 */
class SmmPanelusService implements Seller
{
    ...
    {
        $apiName = config('services.sellers.list.smm_panelus.api_name');
        $client  = Http::$apiName();
    ...
    public function add(OrderDto $orderDto)
    {
        $dto = new SendSellerRequestDto([
            'action'   => config('services.sellers.list.smm_panelus.methods.add'),
            'key'      => $this->key,
            'service'  => $orderDto->service->api_service_id,
            'link'     => $orderDto->link,
            'quantity' => $orderDto->quantity,
            'interval' => $orderDto->interval,
        ]);
    ...
    public function order(OrderDto $orderDto): OrderStatusSellerSuccessResponseDto|SellerFailedResponseDto
    {
        $dto = new OrderStatusSellerRequestDto([
            'action' => config('services.sellers.list.smm_panelus.methods.order'),
    ...
    public function orders(Collection|array $orders): Collection
    {
        ...
        $dto = new OrdersStatusSellerRequestDto([
            'action' => config('services.sellers.list.smm_panelus.methods.orders'),
    ...
    public function balance(): BalanceSellerSuccessResponseDto|SellerFailedResponseDto
    {
        $dto = new BalanceSellerRequestDto([
            'action' => config('services.sellers.list.partner_soc.methods.balance'),
            'key'    => $this->key
        ]);
    ...
    public function services(): Collection
    {
        $dto = new ServicesSellerRequestDto([
            'action' => config('services.sellers.list.smm_panelus.methods.services'),
            'key'    => $this->key
        ]);
